...
def list_cities_of_state(state_id):
    ...
    state_object = [obj.to_dict() for obj in all_states if obj.id == state_id]
    if state_object == []:
        abort(404)
...
def updates_city(city_id):
    ...
    all_cities = storage.all("City").values()
    city_object = [obj.to_dict() for obj in all_cities if obj.id == city_id]
    if city_object == []:
        abort(404)
    if not request.get_json():
        abort(400, 'Not a JSON')
    city_object[0]['name'] = request.json['name']
    for obj in all_cities:
        if obj.id == city_id:
            obj.name = request.json['name']
    storage.save()
    return jsonify(city_object[0]), 200
