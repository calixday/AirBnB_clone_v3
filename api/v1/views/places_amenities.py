...
if getenv('HBNB_TYPE_STORAGE') == 'db':
    ...
    def list_amenities_of_place(place_id):
        ...
        place_obj = [obj.to_dict()
                     for obj in all_the_places if obj.id == place_id]
        if place_obj == []:
            abort(404)
        list_amenities = []
        for obj in all_the_places:
            if obj.id == place_id:
    ...
    def delete_place_amenity(place_id, amenity_id):
        ...
        all__the_places = storage.all("Place").values()
        place_obj = [obj.to_dict()
                     for obj in all__the_places if obj.id == place_id]
        if place_obj == []:
            abort(404)
