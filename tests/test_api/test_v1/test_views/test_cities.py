...
class TestCities(unittest.TestCase):
    ...
    def test_lists_cities_of_state(self):
        ...
        with app.test_client() as c:
            state = State(name="Beckystan")
            storage.new(state)
            new_city = City(name="Chensville", state_id=state.id)
            storage.new(new_city)
            resp = c.get('/api/v1/states/{}/cities'.format(state.id))
            self.assertEqual(resp.status_code, 200)
            resp2 = c.get('/api/v1/states/{}/cities/'.format(state.id))
            self.assertEqual(resp.status_code, 200)
    ...
    def test_create_city(self):
        ...
        with app.test_client() as c:
            ...
            city = City(name="Chensville", state_id=new_state.id)
            storage.new(city)
